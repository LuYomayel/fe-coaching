...
export default function ManageStudentsPage() {
  ...
  if (!userContext) {
    throw new Error('ManageStudentsPage must be used within a UserProvider');
  }
  ...
  }, [refreshKey, user.userId, showToast, coach.id]); // Removed setLoading to prevent infinite loop
  ...
  const openSubscriptionDialog = (student) => {
    if (coachPlans.length === 0) {
      showToast(
        ...
        intl.formatMessage({ id: 'student.error.noCoachPlans' }),
        true
      );
      return;
    }
  ...
  return (
    <div className="manage-students-page p-4">
      ...
      <Dialog
        ...
        style={{ width: '50vw' }}
      >
        <NewStudentDialog onClose={handleNewStudentDialogHide} setRefreshKey={setRefreshKey} studentData={null} />
      </Dialog>
