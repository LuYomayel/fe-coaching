...
export default function ClientProfile() {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [user?.userId, refreshKey, showToast, client]); // Removed setLoading to prevent infinite loop
  ...
  }, [fitnessGoal, activityLevel, personalInfo, intl, showConfirmationDialog, showToast]); // Removed setLoading to prevent infinite loop
  ...
  const statusFilterTemplate = useCallback(
    ...
    [statuses, statusItemTemplate]
  );

  const planNameFilterTemplate = useCallback((options) => {
    return (
      <InputText
        value={options.value}
  ...
  const descriptionFilterTemplate = useCallback((options) => {
    return (
      <InputText
        value={options.value}
        onChange={(e) => options.filterApplyCallback(e.target.value)}
  ...
  const setPlanDetails = useCallback((rowData) => {
    return (
      <p>
        <FormattedMessage
