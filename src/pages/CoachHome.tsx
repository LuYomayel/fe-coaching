...
export default function CoachHomePage() {
  ...
  if (!userContext) {
    throw new Error('CoachHome must be used within a UserProvider');
  }
  const { coach, user } = userContext;
  const router = useRouter();
  const intl = useIntl();

  // === Estados de datos de servicios ===
  const [lastTimeTrainedData, setLastTimeTrainedData] = useState<LastTimeTrainedData[]>([]);
  const [howLongToFinishCycleData, setHowLongToFinishCycleData] = useState<HowLongToFinishCycleData[]>([]);
  const [trainingFrequencyData, setTrainingFrequencyData] = useState<TrainingFrequencyData[]>([]);
  const [paymentStatusData, setPaymentStatusData] = useState<PaymentStatusData[]>([]);

  // === Datos combinados para el dashboard ===
  const [combinedClientData, setCombinedClientData] = useState<CombinedClientData[]>([]);
  ...
  // Usar useRef para evitar re-ejecuciones innecesarias

  useEffect(() => {
    const fetchData = async () => {
      if (!coach?.id || isInitialized) {
        return;
      }
      ...
      try {
        const [lastTimeTrained, howLongToFinishCycle, trainingFrequency, clientsPaymentStatus] = await Promise.all([
          fetchLastTimeTrained(coach.id.toString()),
          fetchHowLongToFinishCycle(coach.id.toString()),
          fetchTrainingFrequency(coach.id.toString()),
          fetchClientsPaymentStatus(coach.id.toString())
        ]);

        setLastTimeTrainedData(lastTimeTrained?.data as LastTimeTrainedData[]);
        setHowLongToFinishCycleData(howLongToFinishCycle?.data as HowLongToFinishCycleData[]);
        setTrainingFrequencyData(trainingFrequency?.data as TrainingFrequencyData[]);
        setPaymentStatusData(clientsPaymentStatus?.data as PaymentStatusData[]);
  ...
  }, [coach?.id, isInitialized]); // Solo depende de coach.id e isInitialized
  ...
  useEffect(() => {
    ...
    const merged: CombinedClientData[] = lastTimeTrainedData.map((lt) => {
      ...
      return {
        ...
        daysLeft: cycleData ? cycleData.daysLeft || null : null,
  ...
  const paymentStatusTemplate = (rowData: CombinedClientData) => {
    return (
  ...
  const lastTrainedTemplate = (rowData: CombinedClientData) => {
    const daysAgo = rowData.lastTimeTrained
      ? Math.floor((new Date().getTime() - new Date(rowData.lastTimeTrained).getTime()) / (1000 * 60 * 60 * 24))
      : null;
  ...
  const daysLeftTemplate = (rowData: CombinedClientData) => {
  ...
  const clientActionsTemplate = (rowData: CombinedClientData) => {
    return (
                ...
                <li key={client.clientId} className="client-item animate-fadeIn">
                  ...
                  <span
                    className={`client-value ${
                      client.daysLeft !== null && client.daysLeft < 3
                        ? 'client-danger'
                        : client.daysLeft !== null && client.daysLeft < 7
                          ? 'client-warning'
                          : 'client-success'
                    }`}
                  >
