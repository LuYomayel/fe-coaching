...
// Tipos extendidos para ClientDashboard
type ExtendedWorkoutInstance = IWorkoutInstance & {
  realEndDate?: string;
  sessionTime?: string;
...
type ExtendedClient = IClient & {
  profilePicture?: string;
};

type ExtendedTrainingCycleTemplate = ITrainingCycleTemplate & {
  name: string;
  duration: number;
...
export default function ClientDashboard() {
  ...
  const [workouts, setWorkouts] = useState<ExtendedWorkoutInstance[]>([]);
  ...
  const [workoutOptions, setWorkoutOptions] = useState<{ label: string; value: number }[]>([]);
  const [filteredWorkouts, setFilteredWorkouts] = useState<ExtendedWorkoutInstance[]>([]);
  ...
  const [cycleOptions, setCycleOptions] = useState<ITrainingCycle[]>([]);
  const [cycleDropdownOptions, setCycleDropdownOptions] = useState<{ label: string; value: number }[]>([]);
  // Prevent duplicate data‑loading caused by React 18 StrictMode
  const lastRefreshRef = useRef<number>(0);
  const isLoadingRef = useRef<boolean>(false);
  const hasSetLoadingRef = useRef<boolean>(false);
  ...
  const [trainingCycleTemplates, setTrainingCycleTemplates] = useState<ExtendedTrainingCycleTemplate[]>([]);
  const [selectedCycleTemplate, setSelectedCycleTemplate] = useState<ExtendedTrainingCycleTemplate | null>(null);
  ...
  useEffect(() => {
    ...
    // Prevenir múltiples ejecuciones del mismo refreshKey
    if (lastRefreshRef.current === refreshKey) {
      console.log('ClientDashboard: skipping fetch, already processed refreshKey:', refreshKey);
      return;
    }
    // Prevenir overlapping de requests
    if (isLoadingRef.current) {
      console.log('ClientDashboard: skipping fetch, already loading');
      return;
    }
    isLoadingRef.current = true;
    lastRefreshRef.current = refreshKey;

    // Solo llamar setLoading(true) una vez por ciclo de carga
    ...
    const fetchAllData = async () => {
      ...
      // Procesar resultado de fetchTrainingCyclesByClient
      if (results[0].status === 'fulfilled') {
        const response = results[0].value as ApiResponse<CalendarEvents>;
        const { events, cycleOptions } = response.data || {};
        if (events && cycleOptions) {
          const mappedEvents = events.map((ev: any) => ({ ...ev, allDay: true }));
          setCycleOptions(cycleOptions);
          setCalendarEvents((e) => [...mappedEvents, ...e]);
          cycleOptions.sort((a: any, b: any) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());
          const options = cycleOptions.map((cycle: any) => ({
            label: `${cycle.name} - ${formatDate(cycle.startDate)} - ${formatDate(cycle.endDate)}`,
            value: cycle.id
          }));
          // Mover intl.formatMessage fuera para evitar bucles infinitos
          const createNewCycleLabel = intl.formatMessage({ id: 'clientDashboard.cycle.createNewCycle' });
          options.unshift({
            label: createNewCycleLabel,
            value: -1
          });
          setCycleDropdownOptions(options);
        }
      } else {
        showToast('error', 'Error fetching training cycles', results[0].reason?.message || 'Unknown error');
      }

      // Procesar resultado de fetchTrainingSessionWithNoWeekByClientId
      if (results[1].status === 'fulfilled') {
        const response = results[1].value as ApiResponse<CalendarEvents>;
      ...
      // Procesar resultado de fetchWorkoutsByClientId
      if (results[2].status === 'fulfilled') {
        const response = results[2].value as ApiResponse<ExtendedWorkoutInstance[]>;
        const data = response.data;
        if (data && Array.isArray(data)) {
          setWorkouts(data as ExtendedWorkoutInstance[]);
          const exercises = [
            ...new Map(
              data
                .flatMap((workout: any) =>
                  workout.groups.flatMap((group: any) =>
                    group.exercises.map((ex: any) => [ex.exercise.id, { id: ex.exercise.id, name: ex.exercise.name }])
                  )
                )
          ...
          setExerciseOptions(exercises.map((ex: any) => ({ label: ex.name, value: ex.id })));

          const uniqueWorkouts = [
            ...new Map(data.map((workout: any) => [workout.workout.id, workout.workout])).values()
          ];
          setWorkoutOptions(
            uniqueWorkouts.map((workout: any) => ({
        ...
        showToast('error', 'Error fetching client workouts', results[2].reason?.message || 'Unknown error');
      }

      // Procesar resultado de fetchClientByClientId
      if (results[3].status === 'fulfilled') {
        const response = results[3].value as ApiResponse<ExtendedClient>;
        const data = response.data;
        if (data) {
          setClientData(data as ExtendedClient);
        }
      } else {
        showToast('error', 'Error fetching client data', results[3].reason?.message || 'Unknown error');
      }

      // Cleanup al finalizar todas las operaciones - solo resetear flags, no llamar setLoading aquí
    ...
    fetchAllData()
      ...
      .finally(() => {
        console.log('fetchAllData finally block');
        // Cleanup final y desactivar spinner
        isLoadingRef.current = false;
        hasSetLoadingRef.current = false;
        setLoading(false);
      });
  }, [clientId, refreshKey, intl]);

  useEffect(() => {
    if (clientData?.coach?.id) {
      fetchTrainingCyclesTemplates(clientData.coach.id)
        .then((response) => {
          const data = response.data;
          if (data) {
            setTrainingCycleTemplates(data as ExtendedTrainingCycleTemplate[]);
          }
        })
        .catch((error) => {
  ...
  useEffect(() => {
    if (selectedExercise) {
            ...
            group.exercises
              ...
              .map((ex) => ({
                ...
                sets: ex.setLogs?.map((set) => ({
                  ...
                  rpe: extractNumber(set.rating) || 0,
      ...
      const processedData = filteredWorkouts.map((fw) => {
        const totalReps = fw.sets?.reduce((sum, set) => sum + set.completedReps, 0) || 0;
        const totalWeight = fw.sets?.reduce((sum, set) => sum + set.weight, 0) || 0;
  ...
  useEffect(() => {
    if (selectedWorkout) {
      ...
      const sortedWorkouts = [...filtered].sort(
        (a, b) => new Date(b.realEndDate || '').getTime() - new Date(a.realEndDate || '').getTime()
      );
  ...
  const handleViewWorkoutDetails = (workoutInstanceId) => {
    console.log('handleViewWorkoutDetails called with:', workoutInstanceId);
    console.log('Current planDetailsVisible:', planDetailsVisible);
    ...
    console.log('After setState - should be visible');

    // Agregar timeout para verificar el estado después del render
    setTimeout(() => {
      console.log('Post-render check - planDetailsVisible:', planDetailsVisible, 'selectedPlan:', selectedPlan);
    }, 100);
  };

  const hidePlanDetails = () => {
    console.log('hidePlanDetails called');
  ...
  const renderEventContent = (eventInfo) => {
    ...
    const renderLocationAndTime = () => {
      ...
      const handleTrainingSessionClick = (e) => {
        ...
        e.stopPropagation();
        router.push(
          `/plans/start-session/${workoutInstanceId}?isTraining=true&planId=${workoutInstanceId}&clientId=${clientId}`
        );
      };
  ...
  return (
    <div className="client-dashboard p-1">
      ...
      {/* Dialog para mostrar detalles del plan */}
      <Dialog
