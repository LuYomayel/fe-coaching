...
export default function CoachProfilePage() {
    ...
    const { showConfirmationDiallog } = useConfirmationDialog();
        ...
        const fetchData = async () => {
            try {
              ...
              setCurrentPlanId(subscriptionData.subscriptionPlan.id);
                console.log(subscriptionData.subscriptionPlan.id)
    ...
    const renderHeader = (text) => {
        ...
        <div className="flex justify-content-between align-items-center">
            ...
            icon="pi pi-plus" 
            onClick={() => text === "Exercises" ? openCreateExerciseDialog() : text === "Workouts" ? navigate('/plans/create') : openCreatePlanDialog()}
            />
        </div>
        );
    };
    ...
    const actionBodyTemplate = (rowData, type) => {
        return (
        <React.Fragment>
            <Button tooltip='View Details' icon="pi pi-eye"    className="p-button-rounded p-button-info p-button-text" 
                onClick={() => {
            if (type === 'exercise') {
                setSelectedExercise(rowData);
  ...
  onChange={(e) => setSelectedBodyAreas(e.value)}  />
    ...
    const renderWorkoutModal = (workoutInstanceId) => {
        return (
            <Dialog draggable={false}  resizable={false} header="Workout details" visible={planDetailsVisible} className="responsive-dialog" style={{ width: '50vw' }} onHide={() => setPlanDetailsVisible(false)}>
                <NewPlanDetail isCoach={true} planId={workoutInstanceId} setPlanDetailsIsVisible={setPlanDetailsVisible} setRefreshKey={setRefreshKey} setLoading={setLoading}/>
            </Dialog>
        );
    };
    ...
    const actionsBodyTemplate = (rowData) => {
        return (
          <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success p-button-sm mr-2" text onClick={() => openEditExerciseDialog(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-danger p-button-sm" text onClick={() => {
    ...
    <div className="coach-profile p-4">
      <Card className="mb-4">
        {coachInfo && (
            ...
            <div className="flex-grow-1">
                ...
                <p className="mb-2"><strong>Email:</strong> {coachInfo.email}</p>
                <p className="mb-2"><strong>Experience:</strong> {coachInfo.experience}</p>
                <p className="mb-2"><strong>Training Type:</strong> {coachInfo.trainingType}</p>
            </div>
            <div className="flex-grow-1 mt-4 md:mt-0">
                <h2 className="text-xl font-bold mb-2">Biography</h2>
                <p>{coachInfo.biography}</p>
            </div>
            </div>
        )}
      </Card>
      ...
      <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
        <TabPanel header="Workouts">
          <DataTable value={workouts} responsiveLayout="scroll" className="p-datatable-sm" header={renderHeader("Workouts")}>
            ...
            <div className="grid">
                ...
                <div key={plan.id} className="col-12 md:col-6 lg:col-4">
                    ...
                    <div className="flex justify-content-between mt-4">
                        <Button label="Edit" icon="pi pi-pencil" className="p-button-text" onClick={() => {openEditPlanDialog(plan)}} />
                        <Button label="Delete" icon="pi pi-trash" className="p-button-danger p-button-text" />
                    </div>
                    </Card>
                </div>
            ))}
      ...
      <Card className="mt-4">
        ...
        <Dialog draggable={false}  resizable={false} header="Workout details" visible={planDetailsVisible} className="responsive-dialog" style={{ width: '50vw' }} onHide={() => setPlanDetailsVisible(false)}>
            <NewPlanDetail isCoach={true} planId={selectedPlan} setPlanDetailsIsVisible={setPlanDetailsVisible} setRefreshKey={setRefreshKey} setLoading={setLoading}/>
        </Dialog>
      {renderExerciseModal()}
      {renderPlanModal()}
      {renderVideoModal()}
