...
export default function TrainingPlanDetails({ setPlanDetailsVisible, setRefreshKey, isTraining = true }) {
  ...
  if (!userContext) {
    throw new Error('TrainingPlanDetails must be used within a UserProvider');
  }
  const { user, client, coach } = userContext;
  ...
  const normalizeExerciseProgress = (exerciseProgress) => {
    const normalizedProgress: ExerciseFeedbackDto[] = [];

    Object.entries(exerciseProgress).forEach(([exerciseId, prog]: [string, IExerciseSetLog]) => {
      const exerciseData = prog;

      let setsArray: IExerciseSetLog[] = [];
      ...
      normalizedProgress[exerciseId] = {
        sets: normalizedSets,
        completed: exerciseData.completed || null,
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      try {
        ...
        const initializeDefaultProgress = () => {
          const initialProgress: IExerciseSetLog[] = [];
          data.groups.forEach((group: IExerciseGroup) => {
            group.exercises.forEach((exercise: IExerciseInstance) => {
              ...
              // Crear un IExerciseSetLog para cada set
              Array.from({ length: numSets }).forEach((_, index) => {
                const setLog = {
                  id: 0,
                ...
                initialProgress.push(setLog);
              });
            });
          });

          setExerciseProgress(initialProgress as ExerciseSetLogDto[]);
        };
        ...
        if (savedProgress) {
          try {
            ...
            Object.entries(parsed).forEach(([exerciseId, prog]) => {
              ...
              // Normalizar los sets
              let setsArray = [];
              if (Array.isArray((prog as any).sets)) {
                setsArray = (prog as any).sets;
              } else if ((prog as any).sets && typeof (prog as any).sets === 'object') {
                setsArray = Object.values((prog as any).sets);
              }
              ...
              normalized[exerciseId] = {
                sets: validSets,
                completed: (prog as any).completed === true ? true : (prog as any).completed === false ? false : null,
                comments: (prog as any).comments || ''
              };
            });

            setExerciseProgress(normalized as any);
          } catch (error) {
    ...
    fetchPlan();
  }, [planId, showToast, setPlanDetailsVisible]); // Removed setLoading to prevent infinite loop
  ...
  const handleExerciseChange = (exerciseId, setIndex, field, value) => {
    setExerciseProgress((prevProgress: any) => {
      if (typeof setIndex === 'number') {
        const prev = (prevProgress as any)[exerciseId] || {};
        const prevSets = Array.isArray((prev as any).sets) ? (prev as any).sets : [];
        ...
        return newProgress as any;
      } else {
        const newProgress = {
          ...prevProgress,
          [exerciseId]: {
            ...(prevProgress as any)[exerciseId],
            [field]: value
          }
        };
  ...
  const handleSetCompletedChange = (exerciseId, setIndex, completed) => {
    setExerciseProgress((prevProgress) => {
      ...
      const allSetsCompleted = Object.values(newProgress[exerciseId].sets).every((set: any) => set.completed);
  ...
  const handleSubmitFeedback = ({ generalFeedback, energyLevel, mood, perceivedDifficulty, additionalNotes }) => {
    ...
    const normalizedProgress: ExerciseFeedbackDto[] = [];
    Object.entries(exerciseProgress).forEach(([exerciseId, prog]: [string, ExerciseFeedbackDto]) => {
      const sets = (prog.sets || []).map((s: any) => ({
        ...s,
    ...
    const exerciseFeedbackArray: ExerciseFeedbackDto[] = Object.entries(normalizedProgress)
      .map(([exerciseId, progress]: [string, ExerciseFeedbackDto]) => {
    ...
    const body: CreateFeedbackDto = {
      workoutInstanceId: parseInt(planId),
      rating: perceivedDifficulty,
      ...
      motivation: mood,
      enjoyment: mood,
      perceivedExertion: mood,
      bodyPartsFocused: [],
      improvements: additionalNotes,
      challenges: additionalNotes,
      overallSatisfaction: mood,
