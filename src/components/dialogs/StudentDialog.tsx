...
interface IStudentDialogProps {
  ...
  studentData: any; // eslint-disable-line @typescript-eslint/no-explicit-any
  visible: boolean;
}

const StudentDialog = ({ onClose, setRefreshKey, studentData, visible }: IStudentDialogProps) => {
  const intl = useIntl();
  const propertyUnits: IPropertyUnits | null = JSON.parse(localStorage.getItem('propertyUnits') || 'null');
