...
// import io from 'socket.io-client'; // TEMPORALMENTE DESACTIVADO
...
import { IMessage } from '../types/shared-types';
...
export default function ChatSidebar({ isCoach }) {
  ...
  if (!userContext) {
    throw new Error('ChatSidebar must be used within a UserProvider');
  }
  const { user, coach, client } = userContext;
  ...
  // const [socket, setSocket] = useState(null); // TEMPORALMENTE DESACTIVADO
  ...
  // Inicializar socket - TEMPORALMENTE DESACTIVADO
  // useEffect(() => {
  ...
  // Manejar mensajes entrantes - TEMPORALMENTE DESACTIVADO
  // useEffect(() => {
  //   if (!socket) return;
  ...
  useEffect(() => {
    if (!isCoach && client?.coach) {
      setSelectedChat(client.coach as any);
    }
    ...
    async (userId, chatUserId) => {
      try {
        ...
        const sortedMessages = fetchedMessages.sort(
          (a: IMessage, b: IMessage) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()
        );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // socket.emit('sendMessage', newMsg); // TEMPORALMENTE DESACTIVADO
      ...
      {selectedChat && (
        <div className="chat-window">
          <div className="chat-header">
            ...
            <Avatar
              //image={`/images/${selectedChat.user.profilePicture}`}
              shape="circle"
              className="mr-2"
              onError={(e: any) => (e.target.src = '/images/default-avatar.png')}
            />
