...
export const useCoachProfileForm = () => {
  ...
  const { user, coach, setCoach } = useContext(UserContext);

  const schema = useMemo(() => buildCoachProfileFormSchema(intl), [intl]);
  const stepFieldMap = [
    ['name', 'bio', 'experience'],
    ['trainingTypes', 'gymLocation'],
  ...
  const applyErrors = useCallback((fields, error) => {
    setFormErrors((prev) => ({
      ...prev,
  ...
  const handleTrainingTypeChange = (event) => {
    setFormValues((prev) => ({ ...prev, trainingTypes: event.value || [] }));
  };

  const handleGymToggle = (event) => {
    setFormValues((prev) => ({
      ...prev,
      hasGym: event.checked,
      gymLocation: event.checked ? prev.gymLocation : ''
    }));
    if (!event.checked) {
  ...
  const handleSubscriptionChange = (value) => {
    setFormValues((prev) => ({
      ...
      planId: value === 'paid' ? (selectedPaidPlan?.id ?? null) : (freeTrialPlan?.id ?? null)
    }));
  ...
  const handleSubmit = async () => {
    ...
    const planDetails = selectedPlan || plans.find((plan) => plan.id === formValues.planId);
    ...
    try {
      ...
      const body = {
        ...
        trainingType: formValues.trainingTypes,
  ...
  const summaryItems = [
    ...
    },
    {
      ...
      value:
        formValues.trainingTypes.length > 0
          ? formValues.trainingTypes.map((type) => trainingTypeOptions.find((opt) => opt.value === type)?.label || type)
