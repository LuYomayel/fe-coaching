...
export const useSignUpDialog = ({ onRequireVerification } = {}) => {
  const intl = useIntl();
  ...
  const [formErrors, setFormErrors] = useState(mapZodErrors(null, ['email', 'password', 'confirmPassword']));
  ...
  const resetForm = () => {
    ...
    setFormErrors(mapZodErrors(null, ['email', 'password', 'confirmPassword']));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (onRequireVerification) {
        onRequireVerification({ email: formValues.email, password: formValues.password });
      }
      close();
      showToast(
        'success',
      ...
      showToast(
        ...
        error?.message || intl.formatMessage({ id: 'home.signup.error.generic' })
      );
    } finally {
      setLoading(false);
    }
