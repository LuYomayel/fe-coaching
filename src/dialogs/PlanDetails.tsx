...
import { IClient } from '../types/shared-types';
...
export default function NewPlanDetailHorizontal({
  ...
  const { user, client } = useContext(UserContext) as UserContextType;
  ...
  const [currentCycle, setCurrentCycle] = useState<ITrainingCycle | -1>(-1);
  const [clientData, setClientData] = useState<IClient | null>(null);
  const [workoutPlan, setWorkoutPlan] = useState<IWorkoutInstance | IWorkoutInstanceTemplate | null>(null);

  // Estado local de loading para evitar conflictos con el componente padre
  ...
  // Ref para prevenir double mounting
  const mountedRef = useRef(false);
  const lastFetchedPlanId = useRef<number | null>(null);
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      // Prevenir fetch si ya se está procesando este planId
      if (lastFetchedPlanId.current === planId) {
        console.log('PlanDetails: skipping fetch, already processed planId:', planId);
        return;
      }

      lastFetchedPlanId.current = planId;
      console.log('PlanDetails: fetching planId', planId);

      try {
        setIsLocalLoading(true);
        const response = isTemplate
          ? ((await fetchWorkoutInstanceTemplate(planId)) as ApiResponse<IWorkoutInstanceTemplate>)
          : ((await fetchWorkoutInstance(planId)) as ApiResponse<IWorkoutInstance>);
    ...
    if (planId && planId !== lastFetchedPlanId.current) {
      fetchPlanDetails();
    }
  ...
  }, [planId, isTemplate]); // ← Removido showToast de dependencias para evitar re-renders

  useEffect(() => {
    // Fetch RPE methods if not a template
    const fetchRpeMethods = async () => {
      try {
        const { data } = await getRpeMethodAssigned(
          clientId,
          planId,
          typeof currentCycle === 'number' ? currentCycle : currentCycle?.id || -1
        );
  ...
  const handleEdit = () => {
    router.push(`/plans/edit/${planId}?changeToTemplate=false&clientId=${clientId}&isEdit=true`);
  };
  ...
  const handleDelete = () => {
    showConfirmationDialog({
      ...
      accept: async () => {
        try {
          const response = await deleteWorkoutPlan(planId, false);
          if (response.message === 'success') {
            showToast(
              ...
              intl.formatMessage(
                ...
                {
                  name:
                    workoutPlan && typeof workoutPlan === 'object' && 'workout' in workoutPlan
  ...
  const handleStartWorkout = () => {
    router.push(
      `/plans/start-session/${planId}?isTraining=true&planId=${workoutPlan?.id}&trainingType=${workoutPlan?.trainingSession?.trainingType}&location=${workoutPlan?.trainingSession?.location}&contactMethod=${workoutPlan?.trainingSession?.contactMethod}&isCoach=${user?.userType === 'coach'}&clientId=${clientId}`
    );
  };
        ...
        const updatedPlan = {
          ...
          trainingSession: prev.trainingSession
            ? {
                ...
                trainingType: editedTrainingType as any
              }
            : undefined
        };
          ...
          <Card className="mb-4">
            <div className="flex justify-content-between align-items-center mb-2">
              <h2 className="text-2xl font-bold m-0">
                ...
                {isTemplate
                  ? (workoutPlan as IWorkoutInstanceTemplate)?.workoutTemplate?.planName
                  : workoutPlan?.instanceName
                    ? workoutPlan?.instanceName
                    : (workoutPlan as IWorkoutInstance)?.workout?.planName}
              </h2>
                  ...
                  {(editedTrainingType === 'presencial' ||
                    ...
                    <div className="col-12 md:col-6 mb-3">
                      ...
                      <Calendar
                        ...
                        onChange={(e) => setEditedSessionTime(e.value || null)}
                        timeOnly
                        hourFormat="24"
                        className="w-full"
                      />
                    ...
                    <div className="flex align-items-center">
                      ...
                      <span>
                        {trainingTypeOptions.find(
                          (option) => option.value === workoutPlan?.trainingSession?.trainingType
                    ...
                    <div className="flex align-items-center">
                      ...
                      <span>
                        {contactMethodOptions.find(
                          (option) => option.value === workoutPlan?.trainingSession?.contactMethod
                ...
                <>
                  ...
                  {!isTemplate && !isEditing && (
                    <Button
                      ...
                      onClick={() => {
                        router.push(
                          `/plans/edit/${workoutPlan?.id}?isEdit=true&changeToTemplate=true&clientId=${clientId}`
                        );
                      }}
                    />
                  )}
