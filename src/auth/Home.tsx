...
import { ICustomJwtPayload } from '../types/shared-types';
...
export default function HomePage() {
  ...
  const setClient = userContext?.setClient;
  const setCoach = userContext?.setCoach;
    ...
    if (token) {
      ...
      if (!decodedToken.isVerified) {
        ...
        setUser({
          ...
          userType: (decodedToken as any).userType || 'coach',
          isVerified: (decodedToken as any).isVerified || false,
          exp: decodedToken.exp || 0,
          name: (decodedToken as any).name || ''
        });
        if ((decodedToken as any).userType === 'client') {
          router.push('/student');
        } else {
          router.push('/coach');
        }
    ...
    if (validateLoginForm()) {
      ...
      try {
        ...
        if (loginData.access_token) {
          ...
          const decodedToken = jwtDecode<ICustomJwtPayload>(loginData.access_token);
          setUser({
            userId: decodedToken.userId || '',
            email: decodedToken.email || '',
            userType: decodedToken.userType || 'coach',
